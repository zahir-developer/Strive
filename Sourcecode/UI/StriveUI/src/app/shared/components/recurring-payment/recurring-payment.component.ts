...
export class RecurringPaymentComponent implements OnInit {
  ...
  paymentProcess() {
    const amount = this.decimalPipe.transform(this.paymentForm.value.Amount, '.2-2');
    ...
    const billingDetailObj = {
      name: null,  // need too change this.billingForm.value.firstName + '' + this.billingForm.value.lastName,
      address: null,  // need too change this.billingForm.value.address1 ? this.billingForm.value.address1 : null,
      city: null,  // need too change
      country: null,  // need too change
      region: null,  // need too change
      postal: null,  // need too change this.billingForm.value.zip ? this.billingForm.value.zip : null// '11111' //
    };
    const authObj = {
      cardConnect: {},
