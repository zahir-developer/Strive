...
-- =============================================
CREATE PROCEDURE [StriveCarSalon].[uspGetHourlyWashReport] @locationId INT, @fromDate Date, @endDate Date
AS
BEGIN
...
Select JobDate,CONVERT(varchar(15), DATEPART(HOUR,CONVERT(datetime, SWITCHOFFSET(CONVERT(datetimeoffset, ActualTimeOut), 
                            DATENAME(TzOffset, SYSDATETIMEOFFSET()))) ),100)as [Hour] into #WashHours
...
from #WashHourResult Group by jobdate
