...
CREATE PROCEDURE [StriveCarSalon].[uspSoftDeleteRestrictReturnTable]
--DECLARE
...
AS
--EXEC [StriveCarSalon].[uspSoftDeleteRestrictReturnTable] 'StriveCarSalon','TblLocation',1

BEGIN
SET NOCOUNT ON;

DECLARE @PRIMARYKEYCOLUMN VARCHAR (200), 
...
DROP TABLE IF EXISTS #DEPENCYRESULT
CREATE TABLE #DEPENCYRESULT (TABLENAME VARCHAR (200),RECORDCOUNT INT)
...
--PRINT @MSG
SELECT *,@PRIMARYKEYCOLUMN FROM #DEPENCYRESULT WHERE RECORDCOUNT>0
