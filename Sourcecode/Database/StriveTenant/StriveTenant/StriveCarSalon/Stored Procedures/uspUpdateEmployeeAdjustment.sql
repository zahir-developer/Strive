...
CREATE  procedure [StriveCarSalon].[uspUpdateEmployeeAdjustment] --[StriveCarSalon].[uspUpdateEmployeeAdjustment] 105,'122.50' 
...
DECLARE @AdjustmentLiabilityId INT =(SELECT valueid FROM GetTable('LiabilityType') WHERE valuedesc='Adjustment');
 ...
 begin
    UPDATE tbleld set tbleld.Amount=@Adjustment,tbleld.IsActive=1,tbleld.IsDeleted=0 FROM tblEmployeeLiability tblel INNER JOIN
