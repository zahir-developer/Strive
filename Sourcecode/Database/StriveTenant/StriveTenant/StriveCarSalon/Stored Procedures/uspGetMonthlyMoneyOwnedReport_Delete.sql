...
CREATE procedure [StriveCarSalon].[uspGetMonthlyMoneyOwnedReport_Delete] --'2020-11-27'
(@Date varchar(7))
AS
BEGIN
...
SELECT tbll.LocationId, tblc.ClientId, COUNT(tblc.FirstName) CustomerCount  
...
GROUP BY tbll.LocationId,tblc.ClientId
...
Select loc.LocationId,mc.ClientId, j.JobDate,SUM(tblji.Price) as TotalJobAmount 
...
twc.AccountAmount,twc.WashesAmount,twc.NumberOfWashes,cc.CustomerCount,CAST((twc.AccountAmount/twc.NumberOfWashes) AS decimal(9,2))Average
,mcoof.TotalJobAmount
from #TotalWashCount twc LEFT JOIN #CustomerCountForLocation cc ON(twc.ClientId = cc.ClientId AND twc.LocationId = cc.LocationId)
