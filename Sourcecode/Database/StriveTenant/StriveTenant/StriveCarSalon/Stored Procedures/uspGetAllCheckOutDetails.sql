...
CREATE   procedure [StriveCarSalon].[uspGetAllCheckOutDetails]--[StriveCarSalon].[uspGetAllCheckOutDetails] 1,1,5,'ASC',NULL
@locationId int =null,
@PageNo INT = NULL,
@PageSize INT = NULL
...
LEFT JOIN
	tblJobPayment tbljp  WITH(NOLOCK) ON tblj.JobId = tbljp.JobId AND tbljp.IsProcessed=1 AND ISNULL(tbljp.IsRollBack,0)=0 AND tbljp.IsActive = 1 AND ISNULL(tbljp.IsDeleted,0)=0 
LEFT JOIN
	GetTable('PaymentStatus') ps ON(tbljp.PaymentStatus = ps.valueid)
LEFT JOIN 
    #PaymentNeedToDone pntd ON(tblj.JobId = pntd.JobId)
WHERE ( @locationId is null or tblj.LocationId =@locationId ) and 
...
select count(*) as Count from tblJob where ISNULL(IsDeleted,0) = 0 
	END