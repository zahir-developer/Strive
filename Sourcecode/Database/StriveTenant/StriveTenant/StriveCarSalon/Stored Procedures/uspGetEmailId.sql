...
inner join tblTimeClock tc on tc.EmployeeId = e.EmployeeId


where (rm.RoleMasterId =@ManagerId or rm.RoleMasterId =@OperatorId) and ea.email IS NOT NULL and (ea.email != ' ' and ea.email like '%@%.%') 
and (el.LocationId in (select Id from split(@LocationId)) or @LocationId Is null)
and (tc.EventDate between @startDate and @endDate)
