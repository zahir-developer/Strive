...
CASE WHEN RoleName='Wash' THEN ISNULL(LoginTime,0) ELSE 0 END AS TotalWashHours,
CASE WHEN RoleName='Detailer' THEN ISNULL(LoginTime,0) ELSE 0 END AS TotalDetailHours,
CASE WHEN RoleName != 'Wash' AND RoleName != 'Detailer' THEN ISNULL(LoginTime,0) ELSE 0 END AS OtherHours
FROM #Hours_Data
)
...
SELECT 	TimeClockId, EmployeeId, RoleId, rm.RoleName, InTime, OutTime
