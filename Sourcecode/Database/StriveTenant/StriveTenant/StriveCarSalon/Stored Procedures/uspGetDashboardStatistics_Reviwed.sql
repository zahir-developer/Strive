...
CREATE PROCEDURE [StriveCarSalon].[uspGetDashboardStatistics_Reviwed] --0,'2020-11-01','2020-11-20'
...
SELECT 
	tblL.LocationId,
	tblTC.EventDate,
	COUNT(tblTC.EmployeeId) Washer,
	COUNT(tblJ.JobId) CarCount
INTO 
	#WashRoleCount 
FROM 
	tblTimeClock tblTC 
INNER JOIN
	tblLocation tblL 
ON	tblTC.LocationId = tblL.LocationId
INNER JOIN tblJob tblJ 
ON	tblJ.LocationId = tblL.LocationId
WHERE -- Add EventDate filter and jobDate in where condition
	tblTC.RoleId =@WashRole  
AND tblTc.EventDate>=@FromDate and tblTC.EventDate<=@ToDate
AND tblJ.JobType=@WashId  
AND tblL.IsActive=1 AND ISNULL(tblL.IsDeleted,0)=0 
...
/* Use #WashRoleCount for fetching employeecount */
...
FROM 
	tblJob tblj 
--INNER JOIN --/*Since Jobstatus has wash and detail service, no need other table join*/
--	tblJobItem tblji 
...
--and tblji.IsActive=1 and tbls.IsActive=1 
AND ISNULL(tblj.IsDeleted,0)=0 
--and ISNULL(tblji.IsDeleted,0)=0 and 
...
Select 
	tblj.LocationId,
	SUM(CAST((CAST(DATEDIFF(MINUTE, TimeIn,ActualTimeOut)AS decimal(9,2))/60) AS decimal(9,2))) TotalHoursWashed
INTO 
	#TotalHoursWashed 
FROM 
	tblJob tblj 
