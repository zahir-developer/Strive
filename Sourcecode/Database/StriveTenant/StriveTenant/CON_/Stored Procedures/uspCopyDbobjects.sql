...
SELECT @CMIN=Min(Column_id),@CMax=MAX(Column_id) FROM #DB_Obj_Columns Where Tbl_Rnk=@Tmin
--PRINT @SQL
WHILE @CMax>=@CMIN
BEGIN
	IF @CMAX>@CMIN
	BEGIN
		SET @SQL= @SQL+ (SELECT COL_DESC FROM #DB_Obj_Columns WHERE Tbl_Rnk=@Tmin AND column_id=@CMIN)+' ,'+CHAR(10)
	END
	IF @CMAX=@CMIN
	BEGIN
		SET @SQL= @SQL+ (SELECT COL_DESC FROM #DB_Obj_Columns WHERE Tbl_Rnk=@Tmin AND column_id=@CMIN)+CHAR(10)
...
JOIN 
	SYS.schemas S
On		S.schema_id=O.schema_id
WHERE S.Name=@DEFAULTSCHEMA AND o.name NOT IN ('uspCopyDbobjects','uspUserCreation','uspMasterDataInsert')
