...
CREATE PROCEDURE [CON].[uspGetCashRegisterDetails] 
(
@LocationId int,
...
FROM [CON].[tblCashRegister] CR
INNER JOIN [CON].[tblCodeValue] CV on CV.id = CR.CashRegisterType
LEFT JOIN [CON].[tblCashRegisterBills] CRB on CRB.CashRegisterId = CR.CashRegisterId
LEFT JOIN [CON].[tblCashRegisterCoins] CRC on CRC.CashRegisterId = CR.CashRegisterId
LEFT JOIN [CON].[tblCashRegisterRolls] CRR on CRR.CashRegisterId = CR.CashRegisterId
LEFT JOIN [CON].[tblCashRegisterOthers] CRO on CRO.CashRegisterId = CR.CashRegisterId

WHERE CR.LocationId=@LocationId
AND CV.id = @CodeValueID
AND CONVERT(date, CR.CashRegisterDate) = @EnteredDate
AND CR.DrawerId in (SELECT DrawerId FROM [CON].[tblDrawer] WHERE LocationId=@LocationId) ORDER BY CR.CashRegisterDate DESC
END