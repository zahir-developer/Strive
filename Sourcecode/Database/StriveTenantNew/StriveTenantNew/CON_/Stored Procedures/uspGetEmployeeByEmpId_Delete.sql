...
CREATE PROCEDURE [CON].[uspGetEmployeeByEmpId_Delete] 
(@EmployeeId int)
as
begin
...
tblemploc.LocationId            AS EmployeeDetail_LocationId,
...
from [CON].tblEmployee tblemp inner join 
[CON].tblEmployeeDetail tblempdet on(tblemp.EmployeeId = tblempdet.EmployeeId) inner join 
[CON].tblEmployeeAddress tblempadd on(tblemp.EmployeeId = tblempadd.EmployeeId) inner join
[CON].tblEmployeeRole tblemprol on (tblemp.EmployeeId = tblemprol.EmployeeId) inner join
[CON].tblEmployeeLocation tblemploc on (tblemp.EmployeeId = tblemploc.EmployeeId)
