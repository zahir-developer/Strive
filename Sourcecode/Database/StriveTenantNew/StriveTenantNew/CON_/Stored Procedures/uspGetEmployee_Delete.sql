...
CREATE PROCEDURE [CON].[uspGetEmployee_Delete]
as
begin
...
[CON].tblEmployeeRole tblemprol on (tblemp.EmployeeId = tblemprol.EmployeeId)
AND
tblemp.IsDeleted=0 AND tblempdet.IsDeleted=0 AND tblempadd.IsDeleted=0 AND tblemprol.IsDeleted=0
Inner Join [CON].tblEmployeeLocation tblemploc on (tblemp.EmployeeId = tblemploc.EmployeeId)
end