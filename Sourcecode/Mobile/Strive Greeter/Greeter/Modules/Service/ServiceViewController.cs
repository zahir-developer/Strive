...
namespace Greeter
{
    public partial class ServiceViewController : BaseViewController, IUITextFieldDelegate
    {
        ...
        async Task CheckBarcodeAndNavigateToViewIssue(string barcode)
        {
            var clientAndVehicleDetail = await CheckBarcodeAsync(barcode);
            var apiService = new GeneralApiService();

            var makesResponse = await apiService.GetAllMake();
            var make = makesResponse.MakeList.Find(x => x.ID == clientAndVehicleDetail.MakeID);
            Make = make.Name;
            if (clientAndVehicleDetail is not null)
            {
                
                ClientID = clientAndVehicleDetail.ClientID;
                VehicleID = clientAndVehicleDetail.VehicleID;
                Barcode = txtFieldBarcode.Text;
                Email = clientAndVehicleDetail.Email;
                MakeID = clientAndVehicleDetail.MakeID;
                ModelID = clientAndVehicleDetail.ModelID;
                UpchargeID = clientAndVehicleDetail.UpchargeID;
                Model = clientAndVehicleDetail.Model;
                ColorID = clientAndVehicleDetail.ColorID;
                txtFieldBarcode.Text = string.Empty;
                NavigateToIssue();
            }
        }
        ...
        void NavigateToIssue()
        {
            ...
            var vc = (IssuesViewController)GetViewController(GetHomeStorybpard(), nameof(IssuesViewController));
        ...
        void NavigateToWashOrDetailScreen(ClientAndVehicleDetail clientDetail = null, string barcode = null)
        {
            ...
            else if (clientDetail != null)
            {
                ...
                vc.ClientEmail = clientDetail.Email;
