...
namespace Strive.ResourceAccess
{
    public class CollisionRal : RalBase
    {
        ...
        {
            return db.Fetch<CollisionViewModel>(EnumSP.Collision.USPGETCOLLISIONBYEMPID.ToString(), null);
        }
        ...
        public CollisionViewModel GetCollisionById(int id)
        {
            ...
            var result = db.FetchMultiResult<CollisionViewModel>(EnumSP.Collision.USPGETCOLLISIONBYID.ToString(), _prm);
            return result;
        }
        public List<CollisionViewModel> GetCollisionByEmpId(int id)
        {
            _prm.Add("@EmployeeId", id);
            var result = db.Fetch<CollisionViewModel>(EnumSP.Collision.USPGETCOLLISIONBYEMPID.ToString(), _prm);
            return result;
        }
        public bool DeleteCollision(int id)
        {
            _prm.Add("@tvpEmployeeLiabilityId", id);
            db.Save(EnumSP.Collision.USPDELETECOLLISION.ToString(), _prm);
            return true;
        }
        ...
        public List<VehicleListViewModel> GetVehicleListByClientId(int id)
        {
            ...
            var result = db.Fetch<VehicleListViewModel>(EnumSP.Client.USPGETVEHICLELISTBYCLIENTID.ToString(), _prm);
            return result;
        }
    }
}