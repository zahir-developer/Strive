...
namespace Admin.API.Scheduler
{
    ...
    public class SecPaymentScheduler : IScheduledTask, IJob
    {
        public string Schedule => "*/1 * * * *";
        IDistributedCache _cache;
        IConfiguration _config;
        TenantHelper _tenant = new TenantHelper(null);
        public SecPaymentScheduler(IDistributedCache dcache, IConfiguration config)
        {
            _config = config;
            _cache = dcache;
        ...
        public Task Execute(IJobExecutionContext context)
        {
            try
            {
                new PaymentGatewayBpl(_cache, _tenant).MakeRecurringPayment(1, DateTime.Now);
            }
            catch (Exception ex)
            {
                throw ex;
